...
in_window = [t for t in direct if "17:00" <= t['departure_time'] <= "21:00"]
cheapest = min(in_window, key=lambda t: t['price'])
print(f"Selected: {cheapest['departure_time']} -- EUR{cheapest['price']}")
```
...
Store the final plan text in a variable (e.g., `final_plan`) so it can be referenced and surgically modified during the Phase 3 validation loop without rebuilding from scratch.
